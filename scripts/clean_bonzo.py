...
# lines that are pure automation / boilerplate, not conversational
DROP_RE = re.compile(
    r"^\s*(unsubscribe|reply stop|msg\s*&?\s*data rates|this is an automated)",
    re.I,
)
# signature-only tails we don't want the model to imitate as a "reply"
...
def load_turns(path):
    ...
    for m in hist:
        ...
        if DROP_RE.search(text) or SIG_RE.match(text):
            continue
        if turns and turns[-1]["role"] == role:  # collapse same-speaker run
            turns[-1]["text"] += " " + text
        else:
