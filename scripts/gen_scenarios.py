...
# realistic client "latest message" fragments per situation (voice bank seeds;
# the teacher rewrites/varies — these just set the scene deterministically)
MORTGAGE_OPENERS = [
...
def gen(rng):
    """Yield one scenario record (dict) chosen by weighted case type."""
    case = rng.choices(
    ...
    if case == "qualifying":
        # some prefix of keys answered; next = first unanswered
        k = rng.randint(0, 3)                       # 0..3 already answered
