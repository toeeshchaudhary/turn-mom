...
# any leftover {UPPER_TOKEN} we don't specifically map
...
class Surrogates:
    """Deterministic surrogate generator for one conversation."""
    ...
    def generic(self, tok):
        ...
        if "DATE" in t:
            return "next Tuesday"
        return ""  # unknown token -> drop cleanly
...
def rehydrate_convo(rec):
    ...
    # pick stable names for the two principals
    agent_name = s.name()
    client_name = s.name()

    def sub_text(text):
        # role-aware name slots first
        ...
        # remaining names -> cycle the pool (adjacent names differ)
    ...
    with open(args.out, "w", encoding="utf-8") as out:
        for path in args.files:
            for line in open(path, encoding="utf-8"):
                ...
                # safety: no curly tokens may survive
                blob = json.dumps(rec)
